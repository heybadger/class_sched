(function() {
      ...
      timeSlots.forEach(timeMin => {
          ...
          DAYS.forEach(day => {
              const cellCourses = courses.filter(c =>
                  c.days.includes(day) && getTimeValue(c.start) === timeMin
              );
              let cellHtml = "";
              if (cellCourses.length > 0) {
                  cellCourses.forEach(c => {
                      const sectionPart = c.section ? `<span class="section-name">${c.section}</span>` : "";
                      ...
                      const endTimeFormatted = formatTime(getTimeValue(c.end));
                      cellHtml += `<div class="course-block">${c.name}<span class="del" data-id="${c.id}">×</span><span class="time-range">${timeLabel} - ${endTimeFormatted}</span>${sectionPart}${roomPart}${buildingPart}</div>`;
                  });
              } else {
  ...
  function addCourseFromForm() {
      ...
      if (!name) { alert("Please enter a course name.");
      ...
      courses.push({ id: generateId(), name, section, room, building, days, start, end });
      render();
      nameInput.value = "";
      sectionInput.value = "";
      roomInput.value = "";
  ...
  function clearAll() {
      ...
      if (confirm("Delete all courses from your schedule?")) { courses = [];
          render(); }
  }
  ...
  function showGifPopup() {
      ...
      gifOverlay.innerHTML = `
          <div class="gif-popup-box">
              <img src="https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExc2czbm5tenYyMHBncDFubXhtbmZ6bDMzaWZxaW5tZ2xudXE3ZDFwcSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/TFz9八Id4LJru8/giphy.gif" alt="Happy GIF" />
          </div>
      `;
  ...
  importInput.addEventListener("change", function(e) {
      if (this.files && this.files.length > 0) { importSchedule(this.files[0]);
          this.value = ""; }
  });
